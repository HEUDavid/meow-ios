...
**Changelog:**
- v1.4 — Self-imposed memory budgets removed (2026-07-28, user directive): the ≤ 40 MB / ≥ 50 MB resident-memory ship-blocker gates, the 45 MB peak target, the §4.2 idle-footprint row, and the `stress_rss.rs` RSS-budget assertions (file deleted) are gone. The OS's ~50 MB jetsam limit remains documented as a platform fact; memory is now observed (About/Memory IPC, memstats log line, harness `rss_monitor`, Instruments) rather than checked against budgets. §8.1 rewritten accordingly; §12 risk row and §13 exit criteria updated. The `PacketTunnel.appex` ≤ 20 MB on-disk size gate is unchanged.
...
- **On the dev box:** the `macos-utun-harness` `rss_monitor` flag charts the FFI's RSS curve (with per-flow state-map counts) under real load — logging only, no assertions. The binary-size gate below is the only CI check in this area.
...
|---------------------|-----------------|----------|
| Extension memory limit (iOS NE cap ≈ 50 MB) | CI fails build if `PacketTunnel.appex` stripped binary > 20 MB (on-disk proxy heuristic — see §8.1); resident memory observed via Settings About/Memory, the memstats log line, and the harness `rss_monitor` (§8.1) — no assertion-based memory checks | P0 |
